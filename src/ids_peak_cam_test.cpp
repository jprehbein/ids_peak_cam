  ...
  //img_pub_(it_->advertiseCamera("ids_cam/image_raw",1))
{
   ...
   nh_.param<int>("/exposureTime", exposureTime_,14985);
...
void ids_cam::camOpen()
{
  ...
  for (const auto& descriptor: deviceManager.Devices())
  {
    ...
    {	
    	...
    	//Opening camera with serial number
...
{	
   ...
   //Create IDS image from buffer
   auto dev_image = peak::BufferTo<peak::ipl::Image>(buffer);
   
   //Color correction test:
   
   // Create IDS peak IPL image from buffer
   //const auto dev_image = peak::BufferTo<peak::ipl::Image>(buffer);
 
   // Create IDS peak IPL image for debayering and convert it to RGBa8 format
   //auto dev_image = image.ConvertTo(peak::ipl::PixelFormatName::BGRa8, peak::ipl::ConversionMode::Fast);
 
   // Queue buffer again
   //m_dataStream->QueueBuffer(buffer);
 
   // Apply color correction
   //m_colorCorrectorIPL.ProcessInPlace(dev_image);
