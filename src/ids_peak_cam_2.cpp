  ...
  //img_pub_(it_->advertiseCamera("ids_cam/image_raw",1))
{
   ...
   img_pub_=it_->advertiseCamera("image_raw2",1);
   
   nh_.param<int>("/frameRate", frameRate_,20);
   nh_.param<int>("/exposureTime", exposureTime_,14990);
   nh_.param<std::string>("/Serial_NO", serNo_,"4104657655");
...
void ids_cam::camConfig()
{
  ...
  ROS_INFO("CURRENT FRAME RATE: %f", frameRate_current);
}
